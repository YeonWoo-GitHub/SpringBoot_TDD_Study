...
public class userDao {
    ...
    // Map<> 으로 사용했던 userDb를 DTO List로 변환
    private static List<userDto> userDb = new ArrayList<>();

    // 현재 데이터베이스 테이블(userDb)의 현 상태 반환
    ...
    public boolean validPassword(String userId, String password) {
        ...
        if (search(userId).getPassword().equals(password)) {
    ...
    public userDto update(userDto entity) {
        ...
        int index = -1;
        userDto user = search(entity.getId());
        userDb.indexOf(user);
        ...
        for (i = 0; i < userDb.size(); i++) {
            ...
            // index를 검색하기 위해 userDb를 순회하여 해당 유저 검색
        ...
        if (-1 == index) {
            ...
            // List.set 메소드 이용하여 해당 index를 가진 유저 정보 업데이트
